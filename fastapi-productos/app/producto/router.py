...
def create_producto(producto: schema.ProductoCreate, db: Session = Depends(get_db)):
    return service.create_producto(db=db, producto=producto)

@router.put("/{producto_id}", response_model=schema.ProductoResponse)
def update_producto(producto_id: int, producto: schema.ProductoCreate, db: Session = Depends(get_db)):
